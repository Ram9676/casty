package com.casty.music.backend.utils.potoken
...
class PoTokenGenerator {
    ...
    private suspend fun getWebClientPoToken(videoId: String, sessionId: String, forceRecreate: Boolean): PoTokenResult {
        ...
        val (poTokenGenerator, streamingPot, hasBeenRecreated) =
            webPoTokenGenLock.withLock {
                val shouldRecreate =
                    forceRecreate || webPoTokenGenerator?.isExpired != false || webPoTokenSessionId != sessionId
                ...
                if (shouldRecreate) {
                    ...
                    val generator = webPoTokenGenerator ?: throw PoTokenException("PoToken WebView was not created")
                    val activeSessionId = webPoTokenSessionId ?: throw PoTokenException("PoToken session id was not set")
                    webPoTokenStreamingPot = generator.generatePoToken(activeSessionId)
                    Timber.tag(TAG).d("Streaming poToken generated")
                }

                Triple(
                    webPoTokenGenerator ?: throw PoTokenException("PoToken generator is unavailable"),
                    webPoTokenStreamingPot ?: throw PoTokenException("Streaming poToken is unavailable"),
                    shouldRecreate,
                )
            }
        ...
        Timber.tag(TAG).d("poToken generated successfully")
