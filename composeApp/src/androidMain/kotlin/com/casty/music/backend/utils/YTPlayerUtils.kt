...
import com.casty.music.backend.innertube.NewPipeExtractor
import com.casty.music.backend.innertube.YouTube
import com.casty.music.backend.innertube.models.YouTubeClient
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.ANDROID_CREATOR
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.ANDROID_VR_1_43_32
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.ANDROID_VR_1_61_48
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.ANDROID_VR_NO_AUTH
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.IOS
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.IPADOS
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.MOBILE
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.TVHTML5
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.TVHTML5_SIMPLY_EMBEDDED_PLAYER
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.WEB
import com.casty.music.backend.innertube.models.YouTubeClient.Companion.WEB_CREATOR
...
import com.casty.music.backend.utils.YTPlayerUtils.MAIN_CLIENT
import com.casty.music.backend.utils.YTPlayerUtils.STREAM_FALLBACK_CLIENTS
import com.casty.music.backend.utils.YTPlayerUtils.validateStatus
import com.casty.music.backend.utils.cipher.CipherDeobfuscator
import com.casty.music.backend.utils.potoken.PoTokenGenerator
import com.casty.music.backend.utils.potoken.PoTokenResult
...
object YTPlayerUtils {
    ...
    suspend fun playerResponseForPlayback(
        ...
        if (isUploadedTrack || playlistId?.contains("MLPT") == true) {
            Timber.tag(TAG).d(
                "Uploaded track initial response: status=%s reason=%s videoId=%s streamingDataNull=%s adaptiveFormats=%s",
                mainPlayerResponse.playabilityStatus.status,
                mainPlayerResponse.playabilityStatus.reason,
                mainPlayerResponse.videoDetails?.videoId,
        ...
        if (streamPlayerResponse == null) {
            ...
            if (isUploadedTrack) {
                Timber.tag(TAG).w("Uploaded track playback failed for videoId=%s: all clients failed", videoId)
            }
        ...
        if (streamPlayerResponse.playabilityStatus.status != "OK") {
            ...
            // unavailable streams; Casty cannot recover those without official playback.
            Timber.tag(logTag).e("Playability status not OK: $errorReason")
            if (isUploadedTrack) {
                Timber.tag(TAG).w(
                    "Uploaded track playability failure: status=%s reason=%s videoId=%s",
        ...
        if (isUploadedTrack) {
            Timber.tag(TAG).d(
                "Uploaded track playback resolved: videoId=%s format=%s bitrate=%s",
                videoId,
                format.mimeType,
                format.bitrate,
            )
        ...
        Timber.tag(TAG).e(e, "Playback resolution failed for videoId=%s", videoId)
    }
    /**
    ...
    private fun validateStatus(url: String): Boolean {
        ...
        try {
            ...
            YouTube.cookie?.let { cookie ->
                ...
                Timber.tag(logTag).d("Added authenticated cookie to validation request")
            }
